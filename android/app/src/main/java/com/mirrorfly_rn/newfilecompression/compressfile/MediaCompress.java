...
import com.mirrorfly_rn.newfilecompression.compressionhelper.AudioInput;
import com.mirrorfly_rn.newfilecompression.compressionhelper.AudioRecoder;
import com.mirrorfly_rn.newfilecompression.compressionhelper.BlankAudioInput;
import com.mirrorfly_rn.newfilecompression.compressionhelper.GeneralAudioInput;
import com.mirrorfly_rn.newfilecompression.compressionhelper.InputSurface;
import com.mirrorfly_rn.newfilecompression.compressionhelper.MP4Builder;
import com.mirrorfly_rn.newfilecompression.compressionhelper.Mp4Movie;
import com.mirrorfly_rn.newfilecompression.compressionhelper.OutputSurface;
import com.mirrorfly_rn.newfilecompression.compressionhelper.SharedConfig;
...
public class MediaCompress {
    ...
    private boolean convertVideoInternal(ConvertVideoParams convertVideoParams,
        ...
        try {
            ...
            Log.e("present_time_start", "-----" + endPresentationTime);
                ...
                if (videoIndex >= 0) {
                    ...
                    try {
                        ...
                        if (avatarStartTime >= 0) {
                            extractor.seekTo(avatarStartTime, MediaExtractor.SEEK_TO_CLOSEST_SYNC);
                        } else if (startTime > 0) {
                        ...
                        encoder.start();
                        outputSurface = new OutputSurface("", "", resultWidth, resultHeight, originalWidth, originalHeight, rotationValue, framerate, false);
                            ...
                            if (!inputDone) {
                                ...
                                if (index == videoIndex) {
                                    ...
                                    if (inputBufIndex >= 0) {
                                        ...
                                        //  inputBuf.position(1000);
                                        int chunkSize = extractor.readSampleData(inputBuf, 0);
                                        Log.e("Buffer_chunk", "position" + inputBuf.position() + "++++" + "limit++++" + inputBuf.limit() + "+++cap+++" + inputBuf.capacity() + "+++inputBufIndex+++" + inputBufIndex);
                                        if (chunkSize < 0) {
                                    ...
                                    if (info.size >= 0) {
                                        ...
                                        extractor.advance();
                                        Log.e("presentation_time", "+++" + extractor.getSampleTime());
                                    } else {
                                        info.size = 0;
                                        inputDone = true;
                                    }
                                    if (info.size > 0 && (endTime < 0 || info.presentationTimeUs < endTime)) {
                                        ...
                                        Log.e("presentation_time", "---------DATA_WRITED+++" + extractor.getSampleTime());
                                        if (availableSize != 0) {
                                            if (callback != null) {
                                                if (info.presentationTimeUs - startTime > currentPts) {
                                                    currentPts = info.presentationTimeUs - startTime;
                                                }
                                                lastcapturetime = currentPts;
                                                Log.e("compress_callback", "---------CallBack--1--+++" + currentPts);
                                ...
                                if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                                    ...
                                    if (info.size > 1) {
                                        if ((info.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) == 0) {
                                            ...
                                            Log.e("presentation_time", "---------DATA_WRITED_SECOND_CONDITION+++" + extractor.getSampleTime());
                                            if (availableSize != 0) {
                                                if (callback != null) {
                                                    if (info.presentationTimeUs - startTime > currentPts) {
                                                        currentPts = info.presentationTimeUs - startTime;
                                                    ...
                                                    lastcapturetime = currentPts;
                                                    Log.e("compress_callback", "---------CallBack--2--+++" + lastcapturetime);
                                                    Log.e("compress_callback", "---------CallBack--2--+++currentPts+++" + currentPts);
                                                    callback.didWriteData(availableSize, (currentPts / 1000f) / durationS);
                                                }
                                            }
                        ...
                        Log.e("FILE_UPLOAD", "++++Error 1 +++" + e.toString());
            ...
            Log.e("FILE_UPLOAD", "++++Error 2 +++" + e.toString());
            error = true;
            FileLog.e("bitrate: " + bitrate + " framerate: " + framerate + " size: " + resultHeight + "x" + resultWidth);
            FileLog.e(e);
        } finally {
            Log.e("FILE_UPLOAD", "++++Close+++");
            if (extractor != null) {
                long endtime = extractor.getSampleTime();
                Log.e("presentation_time", "Extractor_FINISH_Time+++" + endtime);
                extractor.release();
            }
            if (mediaMuxer != null) {
                try {
                    filewritePosition = mediaMuxer.getFileWritePosition();
                    Log.e("presentation_time", "file_last_position_GET++++" + filewritePosition);
                    mediaMuxer.finishMovie();
                    endPresentationTime = mediaMuxer.getLastFrameTimestamp(videoTrackIndex);
                    Log.e("present_time_finish", "-----" + endPresentationTime);
                } catch (Throwable e) {
                    FileLog.e(e);
                }
            }
        ...
        Log.e("presentation_time", "---------CALLBACK_SENTED+++" + lastcapturetime);
        Log.e("compress_callback", "---------CallBack--FINAL--+++" + lastcapturetime);
...
//        }
    ...
    private long readAndWriteTracks(MediaExtractor extractor, MP4Builder mediaMuxer,
                                    MediaCodec.BufferInfo info, long start, long end, long duration, File file, boolean needAudio) throws Exception { // NOSONAR

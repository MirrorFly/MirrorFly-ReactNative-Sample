    ...
    @try {
        ...
        for (AVAudioSessionPortDescription *port in ports) {
            if ([port.portName isEqualToString:inputName]) {
                BOOL isOverriden = [myAudioSession overrideOutputAudioPort:AVAudioSessionPortOverrideNone error:&err];
...
- (void)configureAudioSession
{
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
      ...
      configuration.categoryOptions = AVAudioSessionCategoryOptionAllowBluetoothA2DP | AVAudioSessionCategoryOptionDuckOthers | AVAudioSessionCategoryOptionAllowBluetooth | AVAudioSessionCategoryOptionAllowAirPlay;
      NSError *error = nil;
      [audioSession setConfiguration:configuration error:&error];
      if (error) {

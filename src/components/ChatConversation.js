...
const ChatConversation = React.memo(props => {
   ...
   const handleMessageListUpdated = messages => {
      if (replyMsgs && Object.keys(replyMsgs).length !== 0 && messages[replyMsgs.msgId]) {
   ...
   React.useEffect(() => {
      ...
      switch (true) {
         case foundMsg.length > 0:
            ...
            break;
         case foundMsg.length === 0 && selectedMsgs.length > 0 && selectedMsgs[0]?.msgStatus !== 3:
            setMenuItems([
               {
                  label:
                     selectedMsgs[0]?.msgBody.message_type === 'text' || selectedMsgs[0]?.msgBody?.media?.caption
   ...
   const handleMsgSelect = React.useCallback(
      (message, recall = false) => {
         ...
         if (selectedMessagesIdRef.current[message?.msgId]) {
            ...
            setSelectedMsgs(prevArray => prevArray.filter(item => message.msgId !== item?.msgId));
         } else {
            selectedMessagesIdRef.current[message?.msgId] = true;
   ...
   const renderReplyMessageTemplateAboveInput = () => {
      ...
      switch (true) {
         ...
         case message_type === 'image':
            return <ReplyImage replyMsgItems={replyMsgs} handleRemove={handleRemove} />;
         case message_type === 'video':
            return <ReplyVideo replyMsgItems={replyMsgs} handleRemove={handleRemove} />;
         case message_type === 'audio':
            return <ReplyAudio replyMsgItems={replyMsgs} handleRemove={handleRemove} />;
         case message_type === 'file':
            return <ReplyDocument replyMsgItems={replyMsgs} handleRemove={handleRemove} />;
         case message_type === 'contact':
            return <ReplyContact replyMsgItems={replyMsgs} handleRemove={handleRemove} />;
         case message_type === 'location':
            return <ReplyLocation replyMsgItems={replyMsgs} handleRemove={handleRemove} />;
         default:
            return null;
      }
   };
         ...
         <Modal visible={isOpenAlert} onRequestClose={closeAlert}>
            <ModalCenteredContent onPressOutside={closeAlert}>
               <View style={styles.modalContentContainer}>
                  <Text style={styles.modalContentText}>Are you sure you want to clear the chat?</Text>
                  <View style={styles.modalActionButtonContainer}>
                     <Pressable
                        contentContainerStyle={[commonStyles.p_4, commonStyles.marginRight_16]}
                        onPress={closeAlert}>
                        <Text style={styles.modalCancelButtonText}>CANCEL</Text>
                     </Pressable>
                     <Pressable contentContainerStyle={commonStyles.p_4} onPress={clearChat}>

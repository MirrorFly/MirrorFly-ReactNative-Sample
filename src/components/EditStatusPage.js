...
const EditStatusPage = (props) => {
    ...
    const handleStatus = async () => {
        ...
        if (isNetworkConnected) {
            let statusRes = await SDK.setUserProfile(props?.profileInfo?.nickName, props.profileInfo.image, statusContent.trim(), props.profileInfo?.mobileNumber, props.profileInfo?.email);
    ...
    return (
        <>
            ...
            <Stack flex="1" >
                <HStack position={"absolute"} pb="4" left={"0"} right={"0"} bottom="0" alignItems={"center"} justifyContent={"space-evenly"} borderTopColor={"#BFBFBF"} borderTopWidth={statusContent.trim() ? "1" : '0'}  >
                    {statusContent.trim() && <>

...
export const updateRecentChatMessage = (messgeObject, stateObject) => {
   ...
   const { msgType, fromUserId, toUserId, msgId, timestamp, chatType, msgBody, publisherId, profileDetails } =
      messgeObject;
   ...
   if (recentChatNames.indexOf(newChatTo) !== -1) {
      const constructNewMessage = {
         ...messgeObject,
         MessageType: msgType || msgBody?.message_type || '',
      ...
      if (!isActiveChatMsg) {
         const existingUnReadCount = recentChatArray.find(i => i.fromUserId === newChatTo)?.unreadCount || 0;
